...
public class FilmeController : ControllerBase
{
    ...
    public async Task<IActionResult> Put(Guid id, FilmeDTO filmeAtualizado)
    {
        ...
        if (filmeBuscado == null)
            return NotFound("Filne não encontrado!");
    ...
    public IActionResult Delete(Guid id)
    {
        ...
        var pastaRelativa = "wwwroot/imagens";
        var caminhooPasta = Path.Combine(Directory.GetCurrentDirectory(), pastaRelativa);

        if(!String.IsNullOrEmpty(filmeBuscado.Imagem))
        {
            var caminho = Path.Combine(caminhooPasta, filmeBuscado.Imagem);
